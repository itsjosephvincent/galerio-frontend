<template>
    <NuxtLayout name="cms">
        ...
        <Head>
            <Title>CMS - New Trainee | {{ runtimeConfig.appName }}</Title>
        </Head>
        ...
        <div>
            ...
            <div class="py-4">
                <h3 class="text-2xl text-gray-500">New Trainee</h3>
            </div>
...
const selectedTrainingCertificate = certificateStore.getSelectedCertificate
const selectedTrainingCertificateId = selectedTrainingCertificate.id
...
async function saveTrainee() {
    ...
    if (!v$.value.$error) {
        ...
        try {
            const params = {
                training_certification_id: selectedTrainingCertificateId,
            ...
            const response = await traineeService.saveTrainee(params)
            if (response.data) {
                successAlert('Success!', t('saveTraineeData'))
                backToTraineesPage()
