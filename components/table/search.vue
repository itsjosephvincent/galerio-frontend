<template>
    <div class="mb-6">
        <div class="flex items-center w-full flex-wrap mb-3 border-2 rounded-lg border-primary-700 relative px-3">
            <span class="flex items-center gap-x-1 absolute text-gray-800">
                <FilterIcon class="w-4" /> <span>Filter</span>
            </span>
            <div class="flex flex-wrap flex-grow-0 ml-16">
                <div v-for="(filter, index) in state.filters" :key="index"
                    class="bg-primary-700 flex items-center justify-center text-white rounded-full text-sm pl-3 pr-2 py-1 mx-1">
