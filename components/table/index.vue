<template>
    <table>
                ...
                <th v-for="(item, index) in props.columnHeaders" :key="index" :width="props.width">
                    <div class="flex items-center gap-x-2" :class="[
                        ...
                        <div class="flex items-center justify-end" v-if="item.sorter">
                            <SwitchVerticalIcon class="h-5 w-5 cursor-pointer text-white hover:text-gray-200"
                                aria-hidden="true" v-show="item.key !== props.sortData.sortField"
                                @click="$emit('sort', { sort: 'ascend', column: item.key })" />
                            <ArrowSmDownIcon class="h-5 w-5 cursor-pointer text-white hover:text-gray-200"
                                aria-hidden="true"
                            ...
                            <ArrowSmUpIcon class="h-5 w-5 cursor-pointer text-white hover:text-gray-200"
                                aria-hidden="true"
        ...
        <tbody>
            <slot name="body"></slot>
            <tr v-if="props.isLoading || (props.data && props.data.data && props.data.data.length === 0)">
